...
int InfoGainRatioDataSet(int idx, vector<pair<string, vector<string>>>dataSetTemp);
double SplitInfo(map<string, int>NumOfItems, int colSiz);
...
void CreatTree(string name, vector<pair<string, vector<string>>> dataSetTemp, string space)
{
    ...
    else
    {
        ...
        idxReturn = InfoGainRatioDataSet(idxAttributeSelect, dataSetTemp);
                    //age               <=30   data
...
int InfoGainRatioDataSet(int idx,vector<pair<string, vector<string>>>dataSetTemp)
{
    ...
    for (int i = 0; i < rowSiz; i++)
    {
        ...
        double SplitInfoValue = SplitInfo(NumOfItems, colSiz);
        double GainRatio = GainInfo / SplitInfoValue;
        AllInfoGain.push_back({ GainRatio ,i });
    }
...
double SplitInfo(map<string, int>NumOfItems,int colSiz)
{
    double SplitInfoValue = 0;
    for (auto i :NumOfItems)
    {
        if (i.second == 0)continue;
        SplitInfoValue += -(i.second / (colSiz * 1.0)) * log2(i.second / (colSiz * 1.0));
    }
